...
class DataApiBadRequestException(
        message: String, request: DataRequest,
        override val response: DataResponseError,
        cause: Exception? = null
): DataApiException(message, request, response, cause) {
    val invalidParams: JsonNode = if (response.statusCode != 400 || response.body !is DataResponseErrorDTO)
        jacksonObjectMapper().createObjectNode() else cleanInvalidParamsArray(response.body.message)
